...
public class LogController {
	...
	private static void initLogLevel(){
		try{
			//K2
			File logFile = new File("D:\\WMG_APP\\External_Program\\PDF_Convertor\\resource\\config\\log.properties"); //WMG
	...
	private static synchronized void write(String logType, String[] msg) throws Exception{
		...
		if (ERROR.equals(logType)){
			...
			email.setSubject(EmailStaff.getEmailStaff("Error_Log_Prefix").trim() + CommonUtil.getProperty(logProperties, "log_email_title"));
			email.setSender(EmailStaff.getEmailStaff("Error_Log_FROM"));
